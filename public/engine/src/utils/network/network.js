...
class NetworkManager {
    ...
    host(customId = null) {
        ...
        this.peer.on('open', (id) => {
            console.log(`[Network] Servidor alojado. Esperando rivales en el ID: ${id}`);
            this._emit('ready', id);
        });
        ...
        this.peer.on('connection', (conn) => {
            if (this.connection && this.connection.open) {
                console.warn('[Network] Alguien intentó unirse, pero la sala ya está llena (1v1).');
                conn.close();
                return;
            }
    ...
    send(data) {
        if (this.connection && this.connection.open) {
            ...
            console.warn('[Network] No se puede enviar datos, no hay una conexión abierta.');
        }
    }
