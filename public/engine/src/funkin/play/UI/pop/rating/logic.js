...
class RatingLogic {
    static preload(scene) {
        ...
        if (scene.cache.json.exists(jsonKey)) {
            ...
            scene.load.once(`filecomplete-json-${jsonKey}`, (k, t, data) => loadRatings(data));
        }
    }
    ...
    onNoteHit(data) {
        ...
        if (!ratingData) {
            console.warn(`[RatingLogic] No se encontró configuración en el JSON de skins para: ${actualRating}`);
            return;
        }
        ...
        if (!this.scene.textures.exists(cacheKey)) {
            console.warn(`[RatingLogic] La imagen no está en caché. Clave buscada: ${cacheKey}`);
            return;
        }
        ...
        const sprite = new window.RatingSprite(this.scene, posX, posY, cacheKey, scaleVal);

        if (window.Preferences && window.Preferences.popUpAnim === 'stackeable') {
            const playerKey = isOpponentNote ? 'p2' : 'p1';

            if (this.activeRatings[playerKey] && this.activeRatings[playerKey].active) {
