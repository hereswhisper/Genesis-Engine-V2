...
class HoldCover extends Phaser.GameObjects.Sprite {
    constructor(scene, strumTarget) {
        ...
        if (!holdData || !holdData.directions || !holdData.directions[strumTarget.direction]) {
        ...
        if (strumTexture && strumTexture.key !== "__MISSING") {
            ...
            if (staticFrameName) {
                ...
                this.staticStrumWidth = frameData.sourceSize ? frameData.sourceSize.w : frameData.width;
                this.staticStrumHeight = frameData.sourceSize ? frameData.sourceSize.h : frameData.height;
            }
        }

        if (holdData.chromaKey && window.StageProps) {
            window.StageProps.applyChromaKey(this, holdData.chromaKey.color || holdData.chromaKey);
        }
        if (holdData.blendMode) {
            this.setBlendMode(Phaser.BlendModes[holdData.blendMode.toUpperCase()] || Phaser.BlendModes.NORMAL);
        }
    ...
    createAnimations(atlasKey) {
        ...
        ['start', 'hold', 'end'].forEach(state => {
            ...
            const validFrames = texture.getFrameNames().filter(f => f.startsWith(prefix) || f.trim().toLowerCase().replace(/\s+/g, "").startsWith(cleanPrefix)).sort();
    ...
    syncPos() {
        ...
        const currentScaleX = strum.staticScaleX !== undefined ? strum.staticScaleX : (strum.scaleX !== undefined ? strum.scaleX : 1);
        const baseStrumScale = strum.baseScale || this.scene.referee.skins.get("gameplay.strumline.scale") || 1;
        const relativeScale = currentScaleX / baseStrumScale;

        const baseCoverScale = this.holdData.scale !== undefined ? this.holdData.scale : 0.7;
