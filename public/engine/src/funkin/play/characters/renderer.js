...
class CharacterRenderer extends Phaser.GameObjects.Sprite {
  constructor(scene, charId, role) {
    ...
    const fw = this.frame.sourceSize ? this.frame.sourceSize.w : this.frame.width;
    const fh = this.frame.sourceSize ? this.frame.sourceSize.h : this.frame.height;
  ...
  applyStageConfig() {
    ...
    const charScale = (this.charData && this.charData.scale !== undefined) ? this.charData.scale : 1;
    const stageScale = cfg.scale !== undefined ? cfg.scale : 1;
  ...
  playSingAnim(direction, isMiss = false, sustainLength = 0) {
    ...
    // FIX: El temporizador ahora suma la longitud del sustain más la cantidad de beats de 'sing_duration'.
    // Cuando el sustain acabe, se descontarán los beats restantes hasta el idle.
    const beatMs = window.Conductor ? window.Conductor.beatLengthMs : 500;
    const singDuration = (this.charData.sing_duration !== undefined && this.charData.sing_duration !== null)
                           ? Number(this.charData.sing_duration)
                           : 4; // Por defecto durará 4 beats en FNF (1 Measure completo)
                           
    this.holdTimer = sustainLength + (beatMs * singDuration);
  }
  dance() {
